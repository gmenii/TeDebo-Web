...
export default async function handler(req, res) {
  ...
  try {
    ...
    const connections = await firestore.collection("payment_connections").limit(100).get();
    ...
    for (const connectionDoc of connections.docs) {
      ...
      try {
        ...
        const accountRef = firestore.collection("shared_accounts").doc(reference.slug);
        await firestore.runTransaction(async (transaction) => {
          const snapshot = await transaction.get(accountRef);
          const account = snapshot.data();
          if (!account || account.creatorUid !== connectionDoc.id) return;
          const participants = [...(account.participants || [])];
          const participant = participants.find((entry) => entry.id === reference.participantId);
